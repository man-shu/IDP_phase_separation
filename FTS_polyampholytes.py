...
class PolySol:
    def __init__(self, sigma, rhop0, lB, v, a, Nx=32, dx=None):
        ...
        self.dx = (
            self.a if dx is None else dx
        )  # distance between two n.n. grid points
        ...
        ks1d = (
            2.0 * np.pi * np.fft.fftfreq(self.Nx, self.dx)
        )  # k's in 1D reciprocal space
        self.kz = np.tile(
            ks1d, (self.Nx, self.Nx, 1)
        )  # 3D array with kz[i,j,l] = ksld[l]
        self.kx = np.swapaxes(
            self.kz, 0, 2
        )  # 3D array with kx[i,j,l] = ksld[i]
        self.ky = np.swapaxes(
            self.kz, 1, 2
        )  # 3D array with ky[i,j,l] = ksld[j]
        self.k2 = (
            self.kx * self.kx + self.ky * self.ky + self.kz * self.kz
        )  # 3D array of k*k

        self.Gamma = np.exp(-self.k2 * self.a**2 / 2.0)  # Gaussian smearing
        self.Prop = np.exp(-self.k2 / 6.0)  # Gaussian chain n.n propagator

        self.GT2_w = (
            self.k2 * self.a**2 / 3.0 - 1.0 / 2.0
        ) * self.Gamma  # smearing in pressure
        self.GT2_psi = (
            self.k2 * self.a**2 / 3.0 - 1.0 / 6.0
        ) * self.Gamma  # smearing in pressure
        ...
        Tcc = np.array(
            [np.sum(Mcc.diagonal(n) + Mcc.diagonal(-n)) for n in range(self.N)]
        )
        Tcc[0] /= 2
        ...
        Tmc = np.array(
            [np.sum(Mmc.diagonal(n) + Mmc.diagonal(-n)) for n in range(self.N)]
        )
        Tmc[0] /= 2
        ...
        self.rhop = (
            np.zeros((self.Nx, self.Nx, self.Nx), dtype=complex) + self.rhop0
        )
    ...
    def calc_densities(self):
        ...
        PSI = 1j * (
            np.tensordot(np.ones(self.N), w_s, axes=0)
            + np.tensordot(self.sig, psi_s, axes=0)
        )

        self.qF = self.qF.copy()
        self.qB = self.qB.copy()
        ...
        for i in range(self.N - 1):
            # forwards propagator
            self.qF[i + 1] = np.exp(-PSI[i + 1]) * ift(
                self.Prop * ft(self.qF[i])
            )
            # backwards propagator
            j = self.N - i - 1
            self.qB[j - 1] = np.exp(-PSI[j - 1]) * ift(
                self.Prop * ft(self.qB[j])
            )
    ...
    def get_pressure(self):
        ...
        lap_qB = np.array(
            [self.lap(np.exp(PSI[i]) * self.qB[i]) for i in range(self.N)]
        )
        term1 = np.sum(self.qF * lap_qB) / (9.0 * self.Q)

        term2 = 1j * np.sum(
            np.array([qs[i] * ift(self.GT2_w * ft_w) for i in range(self.N)])
        )
        term3 = 1j * np.sum(
            np.array(
                [
                    qs[i] * self.sig[i] * ift(self.GT2_psi * ft_psi)
                    for i in range(self.N)
                ]
            )
        )

        Pi = (self.rhop0 / self.N) * (
            1.0 - (term1 + term2 + term3) * self.dV / self.V
        )
...
def CL_step_SI(PS, M_inv, dt, useSI=True):
    ...
    dpsi = (
        -dt
        * (
            1j * ift(PS.Gamma * ft(PS.rhoc))
            - PS.lap(PS.psi) / (4.0 * np.pi * PS.lB)
        )
        + eta_psi
    )

    print(
        np.mean(np.abs(PS.w / PS.v)),
        np.mean(np.abs(-PS.lap(PS.psi) / (4.0 * np.pi * PS.lB))),
    )
...
def get_M_inv(PS, dt):
    K11 = PS.Gamma**2 * PS.rhop0 * PS.gmm + 1.0 / PS.v
    K12 = PS.Gamma**2 * PS.rhop0 * PS.gmc
    K22 = PS.Gamma**2 * PS.rhop0 * PS.gcc + PS.k2 / (4.0 * np.pi * PS.lB)
    K11[0, 0, 0] = 1.0 / PS.v

    M = np.array([[1.0 + dt * K11, dt * K12], [dt * K12, 1.0 + dt * K22]])
    ...
    with open(run_label + "_traj.txt", "w") as f:
        for t in range(t_prod):
            ...
            if t % 100 == 0:
                ...
                plt.savefig(f"FTS_polyampholytes_{t}_densities.png")
                plt.close()
