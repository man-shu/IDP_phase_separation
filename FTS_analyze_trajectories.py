...
def find_intersection(rho, mu, Pi):
    ...
    segments = np.array(
        [
            make_segment(Pi[1], mu[1], Pi[0], mu[0]),
    ...
    for i in range(3, len(rho)):
        ...
        for j in range(len(segments) - 1):
            ...
            if len(intersec) > 0:
                ...
                rho_Hs = np.append(rho_Hs, (rho[i] + rho[i - 1]) / 2.0)
                rho_Ls = np.append(rho_Ls, (rho[j] + rho[j - 1]) / 2.0)
    ...
    for i in range(len(rho_Hs)):
        rho_sol = opt.fsolve(
            diff_func,
            [np.log(rho_Ls[i]), np.log(rho_Hs[i])],
            args=(rho, mu, Pi),
        )
    ...
    if show_plot:
        plt.plot(
            mu_c_out, Pi_c_out, "o", color="green", ms=10, fillstyle="none"
        )
        plt.show()
        plt.close()
...
ncpus = 40
all_rho = np.exp(
    np.linspace(np.log(1e-6), np.log(10), ncpus)
)  # All bead bulk densities considered.
all_lB = 1.0 / np.linspace(
    1.0, 6.0, 5
)  # All values of the Bjerrum length (lB).

t_equil = 50.0  # Equilibration Complex Langevin time
...
with open(run_label_base + "_phase_diagram.txt", "w") as pd_file:
    ...
    for lB_ind in range(len(all_lB)):
        ...
        for run in range(ncpus):
            ...
            _, t, mu_R, mu_I, Pi_R, Pi_I = np.loadtxt(
                run_label_base
                + "_lB_"
                + str(lB_ind)
                + "_"
        ...
        out_data = np.array(
            [all_rho, mu_av.real, mu_av.imag, Pi_av.real, Pi_av.imag]
        )
        np.savetxt(
            run_label_base + "_lB_" + str(lB_ind) + "_mu_Pi.txt", out_data.T
        )
        ...
        if len(sol) == 4:
            ...
            pd_file.write(
                "{:.8e} {:.8e} {:.8e} {:.8e} {:.8e}".format(
                    all_lB[lB_ind], rho_L, rho_H, mu_c, Pi_c
                )
            )
            pd_file.write("\n")
