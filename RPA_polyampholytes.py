...
class PolySol_RPA:
    def __init__(self, seq, lB, v, a):
        ...
        epsilon = 1e-7
        n_points = int(1e4)
        self.k = np.linspace(epsilon, 20.0, n_points)
        ...
        for alpha in range(0, self.N):
            for beta in range(0, self.N):
                self.gcc += (
                    self.seq[alpha]
                    * self.seq[beta]
                    * np.exp(-np.abs(alpha - beta) * self.k**2 / 6.0)
                )
                self.gmc += self.seq[alpha] * np.exp(
                    -np.abs(alpha - beta) * self.k**2 / 6.0
                )
                self.gmm += np.exp(-np.abs(alpha - beta) * self.k**2 / 6.0)
    ...
    def set_v_lB(self, v, lB):
        ...
        self.A = Gamma_tilde_squared * (
            self.v * self.gmm + 4.0 * np.pi * self.lB / self.k**2 * self.gcc
        )
        self.B = (
            Gamma_tilde_squared**2
            * 4.0
            * np.pi
            * self.lB
            / self.k**2
            * self.v
            * (self.gmm * self.gcc - self.gmc**2)
        )
    ...
    def calc_mu_Pi(self, rho):
        ...
        mu_integrand = (
            self.k**2
            * (self.A + 2.0 * self.B * rho)
            / (1.0 + self.A * rho + self.B * rho**2)
        )
        mu_fl = self.N * np.sum(mu_integrand) * self.dk / (4.0 * np.pi**2)
        mu_RPA = mu_MFT + mu_fl

        # osmotic pressure
        p_MFT = rho / self.N + self.v * rho**2 / 2.0
        p_integrand = self.k**2 * (
            (self.A * rho + 2.0 * self.B * rho**2)
            / (1.0 + self.A * rho + self.B * rho**2)
            - np.log(1.0 + self.A * rho + self.B * rho**2)
        )
        p_fl = np.sum(p_integrand) * self.dk / (4.0 * np.pi**2)
        p_RPA = p_MFT + p_fl
    ...
    def find_binodal(self, rho_L_in, rho_H_in):
        rho = opt.fsolve(
            self.diff_function, [np.log(rho_L_in), np.log(rho_H_in)]
        )
...
if __name__ == "__main__":
    ...
    plt.savefig("plots/RPA_mu_Pi.png")
    plt.close()
    ...
    plt.savefig("plots/RPA_phase_diagram.png")
    plt.close()
